...
def initialized(key: random.PRNGKey, model: nn.Module):
    ...
    init_batch = random.randint(
            batch_init, shape=(1, model.block_size), maxval=model.vocab_size, minval=0
    )
...
def create_train_state(
    ...
    mask = jax.tree_map(lambda x: x.ndim != 1 and x.shape != (model.block_size,model.embedding_dim), params)

    tx = optax.adamw(
        learning_rate=learning_rate_fn, weight_decay=weight_decay, mask=mask, b2 = 0.95
    )
