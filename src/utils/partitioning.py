...
def set_partitions(in_dict):
    ...
    _unmatched = object()
    initd = {k: _unmatched for k in flatten_dict(in_dict)} # replaces all values in the dict with _object()

    result = {k: replace(k, v) for k, v in initd.items()} # replaces all values in the initd dict with defined PartitionSpec rules

    assert _unmatched not in result.values(), "Incomplete partition spec! All parameters must have a partitioning rule"
